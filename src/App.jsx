...
const InputRow = ({ label, value, onChange, suffix, step = 1000, min = 0, max, hint }) => (
  <div style={{ marginBottom: 14 }}>
    ...
    <div style={{ display: "flex", alignItems: "center", background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 6, overflow: "hidden" }}>
      <input type="number" value={value === 0 ? "" : value} min={min} max={max} step={step}
        placeholder="0"
...
const ArendenView = () => {
  ...
  const buildNtBody = () => {
    const fstyper = [
      ...
      nt.sjukvard && "Sjukvårdsförsäkring",
      nt.sjukOlycka && "Sjuk & Olycksfall",
...
Titel:            ${nt.kontaktTitel}
Telefon:          ${nt.kontaktTel}
E-post:           ${nt.kontaktEmail}

FÖRSÄKRINGSTYPER
${fstyper}

ANSTÄLLDA & LÖN
Antal anställda:  ${nt.antalAnstallda}
Total lönesumma:  ${nt.loneSumma} kr/år
Medellön:         ${nt.medellon} kr/mån
...
FÖRMEDLARE
Namn:             ${nt.formedlarNamn}
Telefon:          ${nt.formedlarTel}
E-post:           ${nt.formedlarEmail}
...
Skickat via Norrfinans Rådgivningsverktyg`;
  ...
  const handleSend = () => {
    ...
    const body = encodeURIComponent(isNt ? buildNtBody() : buildAnBody());
        ...
        <div style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: 12, padding: "28px 28px" }}>
          ...
          {submitted && (
            <div style={{ marginTop: 12, padding: "10px 16px", background: "#ECFDF5", border: "1px solid #6EE7B7", borderRadius: 7, color: "#065F46", fontSize: 12, fontWeight: 600 }}>
              ✓ Outlook öppnas med ärendet förifyllt
            </div>
          )}
        </div>
      )}
...
function MainApp({ onLogout }) {
  ...
  const r = useMemo(() => {
    ...
    const omsättning = intäkterMode === "timbaserat"
      ? timpris * faktTim
      : direktOmsättning;
    const lönTotal = bruttolön * 12 * 1.3142;          // bruttolön × 12 × (1 + AG-avgift 31,42%)
    const pension = pensionMån * 12;
    const pensionSLP = pension * 0.2426;                // Särskild löneskatt på pension 24,26%
              ...
              <div style={{ padding: "24px 20px", background: "#EEF2F7" }}>
                ...
                <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: "18px 20px", marginBottom: 20, boxShadow: "0 1px 4px rgba(15,40,71,0.06)" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0" }}>
                    <div>
                      <div style={{ color: C.text, fontSize: 14, fontWeight: 700 }}>Kvar i bolaget</div>
